...
#ifdef SHELLSIM_CLI
        /* the POSIX bridge (shellsim_libc.c) before libc, so that its
           chmod is the one linked: Preview 1 cannot change permissions */
        if (tcc_add_library(s1, "shellsim") < 0)
            tcc_error_noabort("cannot find libshellsim.a");
#endif
